...
train_feature_resampled, train_label_2_resampled = smote.fit_resample(scaled_train_features, train_label_2)
...
model = SVC()
model.fit(train_reduced_df, train_label_2_resampled)
...
fin_model = SVC(C= 0.1, gamma = 1, kernel = 'poly')
fin_model.fit(train_reduced_df, train_label_2_resampled)
...
# %%
#Write to file
fin_test_pred_df = pd.DataFrame(fin_test_pred, columns=['label_2'])
fin_test_pred_df.to_csv('predictions/label_2.csv', index=False)
